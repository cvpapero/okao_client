...
static const std::string OPENCV_WINDOW = "OKAO Client2 Window";
...
public:
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& imgmsg)
  {
	...
	Mat reRgbImage(tmpRgbImage, cv::Rect(640, 360, 640, 360));
	rgbImage = reRgbImage;
	cv::cvtColor(rgbImage,grayImage,CV_BGR2GRAY);
	//Mat tmpImage(tmpGrayImage, cv::Rect(640, 360, 640, 360));
	//grayImage = tmpImage;
      }
...
{
  ros::init(argc, argv, "OkaoClient2");
