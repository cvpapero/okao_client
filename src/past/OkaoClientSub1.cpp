...
static const std::string OPENCV_WINDOW = "OKAO Client Sub1 Window";
...
public:
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& imgmsg)
  {
	...
	//Mat reRgbImage(tmpRgbImage, cv::Rect(0, 0, 960, 540));
	cv::resize(tmpRgbImage, rgbImage, cv::Size(1280,720));
	//rgbImage = reRgbImage;
...
{
  ros::init(argc, argv, "OkaoClientSub1");
